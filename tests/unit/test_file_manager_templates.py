...
def test_load_template_success(templates_dir):
    template_name = "test_template.md"
    expected_content = "# Test Template\nHello {{ name }}!"
    loaded_content = load_template(str(templates_dir / template_name))
    assert loaded_content == expected_content

def test_load_template_not_found(templates_dir):
    template_name = "non_existent_template.md"
    with pytest.raises(FileNotFoundError):
        load_template(str(templates_dir / template_name))